...
output = "../src/embedded_shaders.js"
shaders = [
    "prefix_sum.comp",
...
for shader in manual_wgsl_shaders:
    with open(shader, "r") as f:
        ...
        compiled_shaders += "export const " + var_name + " = `" + "".join(f.readlines()) + "`;\n";

compiled_shaders = compiled_shaders.replace("type", "alias")
with open(output, "w") as f:
    f.write("export const ScanBlockSize = {};\n".format(block_size))
    f.write("export const SortChunkSize = {};\n".format(sort_chunk_size))
